...
QTSS_Error  QTSSFileModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParamBlock)
{
    switch (inRole)
    {
        ...
        case QTSS_RereadPrefs_Role:
            return RereadPrefs();
        case QTSS_RTSPRequest_Role: // 该角色只由QTSSFileModule模块执行
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params* inParamBlock)
{
	...
	QTSS_Error theErr=QTSS_GetValuePtr(inParamBlock->inRTSPRequest, qtssRTSPReqMethod, 0, (void**)&theMethod, &theMethodLen);
    ...
    switch (*theMethod)
    {
        case qtssDescribeMethod:
        	qtss_printf("-----> QTSSFileModule qtssDescribeMethod (*theMethod) %u\n",(*theMethod));

            err = DoDescribe(inParamBlock);
            break;
        case qtssSetupMethod:
  		    qtss_printf("-----> QTSSFileModule qtssSetupMethod (*theMethod) %u\n",(*theMethod));

            err = DoSetup(inParamBlock);
            break;
        case qtssPlayMethod:
  		    qtss_printf("-----> QTSSFileModule qtssPlayMethod (*theMethod) %u\n",(*theMethod));

            err = DoPlay(inParamBlock);
            break;
        case qtssTeardownMethod:
  		    qtss_printf("-----> QTSSFileModule qtssTeardownMethod (*theMethod) %u\n",(*theMethod));
        ...
        {
  		    qtss_printf("-----> QTSSFileModule qtssPauseMethod (*theMethod) %u\n",(*theMethod));
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params* inParamBlock)
{
	...
	****************************************************************/
    ...
    {
    	/****************************************************************
		*	生成的SDP和发送之前，请检查是否有一个If-Modified-Since日期。
		*	如果有，并且内容没有被修改，那么只返回一个没有修改过的304
    	****************************************************************/
...
QTSS_Error SendPackets(QTSS_RTPSendPackets_Params* inParams)
{
	...
	*****************************************************************/
    ...
    while (true){   
    	...
    	/*****************************************************************
		*	theFile类对象 fPacketStruct.packetData 数据为空
		*	重新获取RTP 轨道列表类
    	*****************************************************************/
...
#if 0
			qtss_printf("*theFile %p theLastPacketTrack %p ",
								*theFile,
								theLastPacketTrack);
#endif
        ...
        *****************************************************************/
		...
		/*****************************************************************
		*	检查是否是视频流,通知QTRTPFile 类视频质量
		*****************************************************************/
        //If the stream is video, we need to make sure that QTRTPFile knows what quality level we're at
        if ( (!sDisableThinning) 
				&& (inParams->inCurrentTime > ((*theFile)->fLastQualityCheck + kQualityCheckIntervalInMsec)) ){
		...
		*****************************************************************/
		...
		if (theStream == NULL)
			return 0;
#if 0
		qtss_printf("-----> theStream  %p "
					"(*theFile)->fNextPacketLen %d \n",
						theStream,
	...
	#if 0
		qtss_printf("curSeqNum %d \n",curSeqNum);
 		qtss_printf("(*theFile)->fNextPacketLen %d \n",
 						(*theFile)->fNextPacketLen);
	#endif
