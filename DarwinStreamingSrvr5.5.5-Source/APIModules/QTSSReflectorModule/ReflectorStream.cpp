...
{
	/* 如果数据流是与RTSP命令交错的(数据流与RTSP都通过RTSPSession)，我们不做任何事 */
...
void ReflectorStream::PushPacket(char *packet, UInt32 packetLen, Bool16 isRTCP)
{
    ...
    if (packetLen > 0){   
		...
		*
		***************************************************************************/
        if (isRTCP){   //qtss_printf("ReflectorStream::PushPacket RTCP packetlen = %lu\n",packetLen);
...
void ReflectorSocket::GetIncomingData(const SInt64& inMilliseconds)
{
    ...
    while (true)
    {
        ...
        qtss_printf("We GetIncomingData \n");              
