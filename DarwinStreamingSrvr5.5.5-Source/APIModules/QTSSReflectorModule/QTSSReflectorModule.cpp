...
QTSS_Error  QTSSReflectorModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
    switch (inRole)
    {
        ...
        case QTSS_RTSPRoute_Role:
            return RedirectBroadcast(&inParams->rtspRouteParams);/* 重定向广播 */
...
QTSS_Error ProcessRTPData(QTSS_IncomingData_Params* inParams)
{
    ...
    /*******************************************************************
    *
    * 获取反射任务 RTSPSession------> ReflectorSession(1:1)
    *
    ********************************************************************/    
...

{
	...
	*/
	/******************************************************************************************
	*	流数据，如RTP数据包的封装（24进制ASCII美元符号），其次是一一字节通道标识，
	*	其次是封装的二进制数据为二进制的长度，网络字节顺序的两个字节的整数。
	*	数据流是紧接着，没有一个CRLF，但包括上层协议头。
	*	每一块美元都包含一个上层协议数据单元，如一个RTP包。
	******************************************************************************************/
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params* inParams)
{
	...
	/***************************************************************************************
	*	(推流端->服务端 命令顺序)  Announce -> SETUP -> PLAY(详细相关资料请百度)
	*	(拉流端->服务端 命令顺序)  DESCRIBE -> SETUP -> PLAY(RTSP 命令拉流标准流程)
	*
	***************************************************************************************/
	#if 0
	qtss_printf("=========== QTSSReflectorModule ================\n");
	#endif
	...
	***************************************************************************************/
    ...
    #if 1
    qtss_printf("QTSSReflectorModule:ProcessRTSPRequest inClientSession=%lu *theMethod %d\n", 
	...
	****************************************************************************************/
    ...
    {
        case qtssPlayMethod:
            return DoPlay(inParams, (*theOutput)->GetReflectorSession()); /* Play 命令(拉流端->服务端) */
...
QTSS_Error DoAnnounce(QTSS_StandardRTSP_Params* inParams)
{
    ...
    ******************************************************************************/
    ...
    QTSSCharArrayDeleter theFullPathStrDeleter(theFullPathStr);
    StrPtrLen theFullPath(theFullPathStr);
	if(theFullPathStr!=NULL){
	    qtss_printf("\nQTSSReflectorModule:DoAnnounce theFullPathStr %s\n\n",theFullPathStr);
	}
	...
	***********************************************************************/
    ...
    //
    // Ok, this is an sdp file. Retreive(检索) the entire contents of the SDP.
    // This has to be done asynchronously(异步) (in case the SDP stuff is fragmented across
	...
	***********************************************************************/
    ...
    //qtss_printf("QTSSReflectorModule:DoAnnounce theContentLenP %s\n",*theContentLenP);
	...
	***********************************************************************/
    ...
    UInt32 theBufferOffset = 0; /* sdp Buffer 内部游标 */
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params* inParams)
{
	...
	***************************************************************/
	...
	#if 1
	qtss_printf("\n\n=========== QTSSReflectorModule DoDescribe================\n\n");
	#endif
	...
	/**************************************************************
	*
	* 如果 RTPSessionOutput 对象已经存在，删除它。
	* 此阶段不允许 RTPSessionOutput 对象存在。
	*
	***************************************************************/
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params* inParams)
{
	...
	*
	* 检查是否是推流
	* *transportModePtr == qtssRTPTransportModeRecord 推流
	* 
	**********************************************************************/
    ...
    if (theLen != sizeof(RTPSessionOutput*))
    {
    	...
    	**********************************************************************/
        ...
        if (theErr != QTSS_NoErr  && !isPush)
        {
        	...
        	/**********************************************************************
			*	这是一个输出流(服务端->拉流端)，创建标准SETUP 应答，
			*	并创建一个RTPSessionOutput 
        	**********************************************************************/
            // This is not an incoming data session...
            // Do the standard ReflectorSession setup, create an RTPSessionOutput
			/********************************************************************** 
			*	设置Session,并创建一个ReflectorSession
			*	同时还会创建 ReflectorSender 类
			**********************************************************************/
            ...
            /**********************************************************************
				创建一个 RTPSessionOutput 并加入到 ReflectorStream 流中
            **********************************************************************/
            RTPSessionOutput* theNewOutput = NEW RTPSessionOutput(inParams->inClientSession, theSession, sServerPrefs, sStreamCookieAttr );
            theSession->AddOutput(theNewOutput,true);
            (void)QTSS_SetValue(inParams->inClientSession, sOutputAttr, 0, &theNewOutput, sizeof(theNewOutput));
       		...
       		/**********************************************************************
			*	输入流(推流端->服务端)，创建一个 ReflectorSession 任务
       		**********************************************************************/
    ...
    **********************************************************************/
    ...
    /******************************************************************************************
	*	是一个数据输入任务(推流端->服务端),除设置 一个新的RTP Stream ,不再执行其他步骤
    ******************************************************************************************/
    ...
    if (isPush){
        ...
        **********************************************************************/
        ...
        /**********************************************************************
		*	数据输入任务(推流端->服务端) 设置完成，跳过其他步骤，直接返回
        **********************************************************************/    
        return QTSS_NoErr;      
    }
    ...
    ***************************************************/
    ...
    {   
        ...
        if (rtpInfoEnabled)
        {
            ...
            {    
 				qtss_printf("ReflectorModule QTSS_Play 1\n");
