...
void RTSPSourceInfo::InitClient(UInt32 inSocketType)
{
    ...
    fClient = NEW RTSPClient(fClientSocket, true, RelaySession::sRelayUserAgent);
}
...
void RTSPSourceInfo::StartSessionCreatorTask(OSQueue* inSessionQueue, OSQueue* inSourceQueue)
{
	...
	***************************************************************/ 
	...
	char strfHostAddr[32];
	SocketUtils::UInt32ToStringIp(fHostAddr,strfHostAddr);
	char strfLocalAddr[32];
	SocketUtils::UInt32ToStringIp(fLocalAddr,strfLocalAddr);
}
    ...
    if (fSessionCreationState == kDone){
        ...
        if(fSession != NULL){
            ...
            for (UInt32 x = 0; x < fNumOutputs; x++){
                ...
                /////////////////////////////////////////////////////////    
				//
				// 控制流程执行完毕，创建反射输出并添加到反射任务中
            	//
            	/////////////////////////////////////////////////////////
                RelayOutput* theOutput = NEW RelayOutput(this, 
                											x, 
                											fSession, 
                											true/* 为true,创建发布(announce)流输出 (重要),服务器模式为先拉后推,详细信息参考开发文档 自动播送*/);
