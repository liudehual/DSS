...
void set_socket_max_buf(int skt)
{
    int i = 1, len;
    len = sizeof(i);
    getsockopt(skt, SOL_SOCKET, SO_SNDBUF, (char*)&i, (socklen_t *)&len);
    ...
    getsockopt(skt, SOL_SOCKET, SO_RCVBUF, (char*)&i, (socklen_t *)&len);
...
int get_interface_addr(int skt)
{
    ...
    err = getsockname(skt, (struct sockaddr*)&localAddr, (socklen_t *)&len);
...
int recv_udp(int socket, char *buf, int amt, int *fromip, int *fromport)
{
    ...
    len = sizeof(sin);
    memset(&sin, 0, sizeof(sin));
    ret = recvfrom(socket, buf, amt, 0, (struct sockaddr*)&sin, (socklen_t *)&len);
    if (ret != -1) {
        if (fromip)
