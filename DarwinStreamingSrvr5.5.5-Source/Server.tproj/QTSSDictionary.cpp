    ...
    fMap(inMap), fInstanceMap(NULL), fMutexP(inMutex), fMyMutex(false), fLocked(false)
{
    if (fMap != NULL){
		...
		#if 0
		qtss_fprintf(stderr,"----------------------> fAttributes size %d \n",inMap->GetNumAttrs());
                                            ...
                                            Bool16 isInternal)
{
	/****************************************************************************************************************** 
	*	
	* 	QTSSDictionaryMap 包含描述 DictValueElement 的元数据及操作方法
	*
	******************************************************************************************************************/
    ...
    /****************************************************************************************************************** 
    *	根据属性ID计算Map 中的索引
	*	非 instance  属性ID 与 Map 中的索引相同
	*	instance 属性ID与Map 中的索引不同(inAttrID = theMapindex | 0x80000000。theMapIndex=inAttrId & 0x7FFFFFFF)
    ******************************************************************************************************************/
    SInt32 theMapIndex = theMap->ConvertAttrIDToArrayIndex(inAttrID);

    if (theMapIndex < 0) /* <0 返回 */
        return QTSS_AttrDoesntExist;
    if (theMap->IsRemoved(theMapIndex)) /* 属性已经被移除，返回 */
        return QTSS_AttrDoesntExist;
	...
	*************************************************************************/
    ...
    /************************************************************************************************* 
    * 
    * theAttrs[theMapIndex].fAttributeData.Len(单个属性长度) * inIndex(属性在data buffer中的索引) 
    * 确定一个属性在data buffer中的位置 
    *
    **************************************************************************************************/  
    theBuffer += theAttrs[theMapIndex].fAttributeData.Len * inIndex;
	#if 0
	qtss_printf("[SELF]================  theAttrs[theMapIndex].fAttributeData.Len %u "
				"inIndex %u\n",
				theAttrs[theMapIndex].fAttributeData.Len,
				inIndex);
	#endif
    /******************************************************************************** 
    *
                                        ...
                                        UInt32 inFlags)
{
    ...
    *	
    * 获取属性个数
    * 问:模块添加静态属性时，是在什么地方申请的DictValueElement结构？
    * 答:系统在初始化完成后才允许创建连接，模块在系统初始化时添加静态变量，
    *	 故在创建实例对象时，所有的属性元数据已经初始化完毕。
    *
    ********************************************************************************************/
    UInt32 numValues = theAttrs[theMapIndex].fNumAttributes;
#if 0
		qtss_fprintf(stderr,"theMapIndex %d numValues %d \n",theMapIndex,numValues);
    ...
    ******************************************/
    if (dataType == qtssAttrDataTypeCharArray){
        ...
        *
        *
        * 该值是迭代的(会存储多个)，新的值的长度必须与其他值的长度相同
        * fAllocatedLen = fNumAttributes(属性个数) * attrLen(每个属性的长度)
		* fAttributeData 格式如下:
	    * ------------------------------------------------------------------
		* |属性1|属性2|属性3|属性4|......
		* ------------------------------------------------------------------
    ...
    /********************************************************************************* 
	*
	* 属性值在内存中的索引不能大于内存中存储的最大属性数
	* inIndex<=numValues
	* 防止属性之间产生空的内存空间
	*   ------------------------------------------------------------------
	*	|属性1| 	空的的空间 |属性2|属性3|属性4|......
	*	------------------------------------------------------------------
	*
	**********************************************************************************/
    ...
    #if 0
   	qtss_printf("[SELF] inIndex %u numValues %u inBuffer %u attrLen %u\n",
						inIndex,
						numValues,
	...
	************************************************************************************/	
	...
	/************************************************************************************ 	
	*
	* 根据 属性长度 * (索引个数+1) 来判断是否重新申请内存
	* 属性长度 * (索引个数+1) > 已申请的内存空间
	*
	************************************************************************************/
    if ((attrLen * (inIndex + 1)) > theAttrs[theMapIndex].fAllocatedLen){
    	/**************************************************************** 
    	*
    	* 内存不足，重新申请，并调整老属性在 fAttributeData 中的位置
    	* 
    	****************************************************************/
...
{ 
	...
	/**********************************************************************************
	*
	* 记录属性的起始位置、数据长度、申请的长度，方便以后读取、设置属性内容
	*
	***********************************************************************************/
...
{
    if (inAttrName == NULL 
			|| ::strlen(inAttrName) > QTSS_MAX_ATTRIBUTE_NAME_SIZE){
        return QTSS_BadArgument;
	}
                                        ...
                                        QTSS_AttrPermission inPermission )
{
	...
	****************************************/
    ...
    /***************************************************************************************************
    * 
    * 创建描述属性的QTSSAttrInfoDict对象(属性元数据)，将属性包含的信息（ID、名称、函数指针、数据类型、
	* 权限）保存下来
    *
    ****************************************************************************************************/ 
	...
	/************************************************************************************
    *
    * 将属性元数据四个成员(名称、ID、数据类型、权限)的相关信息通过基类QTSSDictionary保存起来
    * 
    *
    *************************************************************************************/ 
